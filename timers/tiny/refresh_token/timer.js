...
const { renovarRefreshToken } = require('../../../controllers/ApiTinyController')

async function lerJson() {
  const data = await fs.readFile('./timers/tiny/refresh_token/config.json', 'utf8')
    ...
    if (
      ...
      if (configs.horarios.includes(horarioAtual)) {
        for (const integracao of configs.integracoes) {
          await renovarRefreshToken(integracao)
        }
      }
    }
  } catch {}
